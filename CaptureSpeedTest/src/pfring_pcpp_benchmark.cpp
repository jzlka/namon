...
void handler(RawPacket* packets, uint32_t numOfPackets, uint8_t threadId, PfRingDevice* device, void* userCookie)
{    
    if (++pkts == pktsToRcv)
	    shouldStop=1;
}
...
int benchmark(char *interface)
{
    ...
    try
    {
        ...
        if (!dev->open())   // FIXME dev is opened in promiscuous mode
            throw "open() failed.";

        clock_type::time_point start = clock_type::now();
        if (!dev->startCaptureSingleThread(handler, NULL))
